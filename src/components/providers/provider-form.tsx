      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        ...
        <Card>
          ...
          <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <FormField control={form.control} name="businessName" render={({ field }) => (
                <FormItem><FormLabel>Razón Social / Nombre</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
              )}
            />
            ...
            <FormField control={form.control} name="documentNumber" render={({ field }) => (
                <FormItem><FormLabel>Número de Documento / NIT</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
              )}
            />
            ...
            />
             <FormField control={form.control} name="email" render={({ field }) => (
                <FormItem><FormLabel>Email para Facturación Electrónica</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
              )}
            />
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            />
            <FormField control={form.control} name="icaTariff" render={({ field }) => (
                <FormItem><FormLabel>Tarifa ICA (si aplica)</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
              )}
            />
        ...
        {/* Section 3 */}
        <Card>
            <CardHeader><CardTitle>3. Información de Contacto</CardTitle></CardHeader>
            <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <FormField control={form.control} name="contactName" render={({ field }) => (
                    <FormItem><FormLabel>Nombre Contacto Comercial</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
                )}/>
                ...
                <FormField control={form.control} name="contactEmail" render={({ field }) => (
                    <FormItem><FormLabel>Email Contacto</FormLabel><FormControl><Input {...field} type="email"/></FormControl><FormMessage /></FormItem>
                )}/>
            </CardContent>
        </Card>
        ...
        {/* Section 4 */}
        <Card>
            <CardHeader><CardTitle>4. Información Financiera para Pagos</CardTitle></CardHeader>
            <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 <FormField control={form.control} name="bankName" render={({ field }) => (
                    <FormItem><FormLabel>Entidad Bancaria</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
                )}/>
                ...
                <FormField control={form.control} name="accountNumber" render={({ field }) => (
                 ...
                 <FormField control={form.control} name="beneficiaryName" render={({ field }) => (
                    <FormItem><FormLabel>Nombre del Titular de la Cuenta</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
                )}/>
            </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Adjunta los siguientes documentos en formato PDF (máximo 2MB cada uno).</CardDescription>
          </CardHeader>
          <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {(['rutFile', 'camaraComercioFile', 'estadosFinancierosFile', 'declaracionRentaFile', 'cedulaRepresentanteLegalFile', 'certificacionBancariaFile'] as const).map(fieldName => {
        ...
        {/* Section 6 */}
        <Card>
            <CardHeader><CardTitle>6. SARLAFT y Aceptación</CardTitle></CardHeader>
            <CardContent>
                <FormField
                    control={form.control}
                    ...
                    render={({ field }) => (
                        <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
                            ...
                            <div className="space-y-1 leading-none">
                                ...
                                <FormDescription>
                                    Declaro que los recursos que componen mi patrimonio no provienen de lavado de activos, financiación del terrorismo, narcotráfico, captación ilegal de dineros y en general de cualquier actividad ilícita. Autorizo a Frio Alimentaria S.A.S para que verifique la información suministrada.
                                </FormDescription>
                                <FormMessage/>
                            </div>
                        </FormItem>
